...
export default function Dashboard() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-white font-[Heebo] relative">
      {/* HEADER */}
      <header
        dir="ltr"
        className="bg-gradient-to-l from-[#ff6f00] to-[#b347ff] text-white py-4 px-8 flex items-center justify-between border-b border-gray-200"
      >
        {/* כותרת בשמאל אמיתי */}
        <div className="flex flex-col items-start text-left">
          ...
          <div className="flex items-center gap-2 text-[#ffeedd] text-sm mt-1">
            <div className="flex items-center justify-center w-7 h-7 font-bold text-white bg-[radial-gradient(circle_at_30%_30%,#ffb84d,#ff6f00_60%,#b347ff)] shadow-[0_0_10px_rgba(255,111,0,0.8)] rounded-full">
              AI
            </div>
      ...
      {/* כפתור הוספת דיון חדש - מימין ולמטה מהרקע הלבן */}
      <div ref={menuRef} className="absolute top-[160px] right-[40px]">
        <button
        ...
        {menuOpen && (
          <div className="absolute right-0 mt-2 bg-white text-gray-800 rounded-lg shadow-lg border min-w-[200px] text-right z-10">
      ...
      <main className="pt-40 px-8">
        <div className="text-center text-xl font-bold mb-6 text-gray-700">
          ...
          <tbody>
            {discussions.map((item, idx) => (
              <tr key={idx} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3">
                  <span
                    className={`text-white px-3 py-1 rounded-full text-xs font-bold ${getStatusClass(
                      item.status
                    )}`}
                  >
